...
class ExtraPin:
    def __init__(self, config):
        ...
        if self.board == 'control_board':
            ...
            gcode.register_mux_command("TOGGLE", "DEVICE", self.name, self.cmd_toggleDevice)
            ...
            self.start_cmd = 'gpio mode ' + str(self.sbc_pin) + ' out'
            os.system('gpio mode ' + str(self.sbc_pin) + ' out') # check it!!!!!!!!

        printer.register_event_handler('klippy:ready', self._handle_ready_klippy)
        gcode.register_mux_command("EXTRA_PIN_INFO", "BUTTON", self.name, self.cmd_getInfo)

    def _handle_ready_klippy(self):
        if self.board == 'single_board_computer':
    ...
    def _update_digital(self, value):
        self.mcu_pin.update_digital(int(value))
        return not value
